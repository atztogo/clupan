/******************************************************************************
	    ...
	    Class for finding primitive cell
...
extern "C" int spg_find_primitive(double lattice[3][3], double position[][3],
...
int main(int argc, char *argv[]){
    ...
    int new_n_all_atom = 
        spg_find_primitive(lattice, position2, type2, natom, symprec);
    ...
    for (int i = 0; i < new_n_all_atom; ++i){
        dvector pos(3);
        for (int j = 0; j < 3; ++j){
            pos(j) = position2[i][j];
        }
