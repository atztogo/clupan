...
CMC::CMC(const Input& ip, const MC_initialize& mc_init)
    : MC_common(ip, mc_init){}

CMC::~CMC(){}

void CMC::core(const int& binary){
    if (binary == 1){
        step_sum = 0; 
...
void CMC::find_sqs(const MC_initialize& mc_init){

    const dvector disorder_cf = mc_init.get_disorder_cf();
    const std::string criterion = mc_init.get_criterion();
...
void CMC::gcmc(const MC_initialize& mc_init){

    std::cout << "  energy of initial structure = " << energy << std::endl;

    // mu = mu(B) - mu(A)
    double mu = mc_init.get_mu()[1];
...
void CMC::cmc(const MC_initialize& mc_init){
...
void CMC::cmc_ewald(const MC_initialize& mc_init){

    std::cout << "  energy of initial structure = " << energy << std::endl;

    const double epsilon = mc_init.get_epsilon();
...
dvector CMC::cmc_diff_correlation
...
dvector CMC::gcmc_diff_correlation
(std::vector<int>& spin, const std::vector<int>& atoms, 
...
void MC::find_sqs(const MC_initialize& mc_init){
...
void MC::gcmc(const MC_initialize& mc_init){

    // mu = mu(B) - mu(A)
    std::vector<double> mu = mc_init.get_mu();
...
void MC::cmc_ewald(const MC_initialize& mc_init){
